...
categories: [lecture, wellbeing, descriptives]
image: "images/LMLLOGO.png"
author:
  - name: Gordon Wright
    role: remixer
...
::: callout
This is placeholder/indicative text is copied (and minimally edited)
from Professor Matthew Crump (et al)'s "Answering Questions with Data"
...
1.  Understand the principles of multivariate analysis
2.  Apply principal component analysis (PCA) to psychological data
3.  Interpret the results of a factor analysis
4.  Critically evaluate the use of multivariate techniques in research
    papers
...
::: callout-warning
Be cautious when interpreting factor loadings. Small sample sizes can
...
What is the primary focus of this week's topic?

`r mcq(c("Option 1", "Option 2", answer = "Correct Answer", "Option 4"))`
...
# Plot the results
autoplot(pca_result, data = df, colour = 'steelblue', loadings = TRUE, loadings.label = TRUE)
```
...
::: webex-check
How might you apply this week's concepts in your own research or
practice?

`r longmcq(c("I wouldn't use these concepts", "I'm not sure how to apply them", answer = "I could use these concepts to [specific application]"))`
:::
...
f2 <- rnorm(n)
items <- data.frame(
  item1 = f1 + rnorm(n, sd = 0.5),
  item2 = f1 + rnorm(n, sd = 0.5),
  item3 = f2 + rnorm(n, sd = 0.5),
  item4 = f2 + rnorm(n, sd = 0.5)
)

# Perform EFA
fa_result <- fa(items, nfactors = 2, rotate = "varimax")

# Print results
print(fa_result, cut = 0.3, digits = 2)
```
...
We measured the standard deviation as `r sqrt(5.916)`. Notice this
number fits right in the with differences scores from the mean. All of
the scores are kind of in and around + or - `r sqrt(5.916)`. Whereas, if
...
of `r sqrt(5.916)`, you would get a pretty good summary of the numbers.
...
huge, it's only + or - `r sqrt(5.916)` on average. That's a good
